...
func _process(delta):
	if Input.is_action_just_pressed("v"):
		noclip = !noclip
		$CollisionShape3D2.disabled = noclip
...
func _unhandled_input(event):
		...
		#var em = event as InputEventAction
		#if em.action == "v":
			#noclip = !noclip
...
func exit(target_map_index):
	Singleton.gameWorld.mapBuilder.current_map_index = target_map_index
	Singleton.gameWorld.rebuild_world()
...
func update_light_radius():
	light_radius = Singleton.lantern_level*5 + 10
